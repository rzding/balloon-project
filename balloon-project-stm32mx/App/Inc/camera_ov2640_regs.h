/**
 * @file camera_ov2640_regs.h
 * @brief OV2640 SCCB register init tables for JPEG capture (F9.1b).
 *
 * Script-extracted (not hand-typed) from the ArduCAM Arduino library:
 *   https://github.com/ArduCAM/Arduino  (ArduCAM/ov2640_regs.h)
 * The library's PROGMEM `sensor_reg` {reg,val} pairs, converted to plain C.
 * Each table ends with the {0xFF, 0xFF} sentinel (reg 0xFF is bank-select but
 * its value is never 0xFF, so the pair is unambiguous as a terminator).
 *
 * Apply order for JPEG output (per the ArduCAM examples):
 *   OV2640_JPEG_INIT -> OV2640_YUV422 -> OV2640_JPEG -> a resolution table.
 * Included only by camera.c; static const so the ~290 pairs live in flash.
 */
...
/** One OV2640 SCCB register write. */
typedef struct
{
  uint8_t reg;
  uint8_t val;
...
/** Table terminator: reg == 0xFF && val == 0xFF. */
#define CAM_REG_TABLE_END 0xFFu

static const cam_sensor_reg_t OV2640_JPEG_INIT[] = {
...
static const cam_sensor_reg_t OV2640_320x240_JPEG[] = {
  {0xFF, 0x01}, {0x12, 0x40}, {0x17, 0x11}, {0x18, 0x43}, {0x19, 0x00}, {0x1A, 0x4B},
  {0x32, 0x09}, {0x4F, 0xCA}, {0x50, 0xA8}, {0x5A, 0x23}, {0x6D, 0x00}, {0x39, 0x12},
  {0x35, 0xDA}, {0x22, 0x1A}, {0x37, 0xC3}, {0x23, 0x00}, {0x34, 0xC0}, {0x36, 0x1A},
  {0x06, 0x88}, {0x07, 0xC0}, {0x0D, 0x87}, {0x0E, 0x41}, {0x4C, 0x00}, {0xFF, 0x00},
