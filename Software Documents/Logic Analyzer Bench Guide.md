...
| DRA818V APRS | USART2 + TIM2 PWM + PTT | F10.1 AT; F10.2 AFSK; F10.3 60 s SM; F10.4 dry-run (`APRS_RF_ENABLE=0` → no PTT low; GDB `g_aprs_last_info`) |
...
- **APRS USART2 / PTT / PWM:** F10.1 boot AT @ 9600; F10.2/F10.3 TIM2 CH1 AFSK via SM (`aprs_poll`). Default `APRS_RF_ENABLE=0`: periodic AFSK without PTT low; inspect `g_aprs_last_info` / `g_aprs_ok` under GDB (F10.4). `APRS_RF_ENABLE=1`: PTT low during lead/play/tail (licensed RF only).
